...
function Preference (args)
{
	...
	this.def = args.def;
	this.fields = args.fields || {key: {name: args.name, validators: new Array()}};
	
	this.validators = new Array();
	if (args.validators) for (var i in args.validators) this.validators.push(args.validators[i]);
	...
	this.validate = function (input) { // Check candidate value with all registered validators
		...
		for (var i in this.validators) {
			...
			if (result.status == "FAIL") { status = "FAIL"; break; }
			else if (result.status == "WARN") { status = ((status == "FAIL") ? "FAIL" : "WARN"); }
		}
	...
	this.get = function() { this.init(); return this.value; }
...
var NonEmptyValidator = function(input) {
	if(input) wrapPassMessage();
	else return wrapFailMessage("Must be non-empty");
}

// Type

var IntValidator = function(input) {
	if(isFinite(input) && (input == Math.round(input))) return wrapPassMessage();
	else return wrapFailMessage("Must be an integer");
};

var FloatValidator = function(input) {
	if(isFinite(input)) return wrapPassMessage();
	else return wrapFailMessage("Must be a number");
};
...
var JSONFieldsValidator = function(input, pref) { 
	...
	for (var i in pref.fields){
		if(input[i] == undefined) wrapFailMessage("Wrong JSON structure (user should never see this!)");
	...
	if (status == "FAIL") { return wrapFailMessage(messages);  }
	else if (status == "WARN") { return wrapWarnMessage(messages); }
	else return wrapPassMessage();
};

var BoolValidator = function(input) { 
	if(input === !!input) return wrapPassMessage();
	else return wrapFailMessage("Must be a boolean value (user should never see this!)");
};

// Comparison

var PositiveValidator = function(input) {
	if(input <= 0) return wrapFailMessage("Must be positive");
	else return wrapPassMessage();
};

var GTValidator = function(min) { return function(input) {
	if(input < min) return wrapFailMessage("Must be no less than " + min);
	else return wrapPassMessage();
};};

var LTValidator = function(max) { return function(input) {
	if(input > max) return wrapFailMessage("Must be no more than " + max);
	else return wrapPassMessage();
};};

var BetweenValidator = function(min, max) { return function(input) {
	if((input > max) || (input < min)) return wrapFailMessage("Must be between " + min + " and " + max);
	else return wrapPassMessage();
};};
...
var GTValidatorSeconds = function(min) { return function(input) {
	if(input < min) return wrapFailMessage("Must be no less than " + min/1000 + " second(s)");
	else return wrapPassMessage();
};};

var GTValidatorMinutes = function(min) { return function(input) {
	if(input < min) return wrapFailMessage("Must be no less than " + min/60000 + " minute(s)");
	else return wrapPassMessage();
};};